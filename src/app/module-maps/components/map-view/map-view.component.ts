...
export class MapViewComponent implements AfterViewInit, OnChanges{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    for (i = 0; i < this.housingService.houses.length; i++) {
      if (this.distanceService.calcularDistancia(userPosition, this.housingService.houses[i].direction) <= 2) {
        for (let categoryId of this.checkedCategories) {
          ...
          for (let infoLocation of categoryInfo){
            ...
            if (this.distanceService.calcularDistancia(this.housingService.houses[i].direction, infoLocation.direction) <= 2) {
